...
export class Battle {
  ...
  constructor(
    ...
    if (players.length > maxPlayerCount)
      throw new Error("Max player count exceeded");
    this.id = id;
  ...
  addPlayer(player: Player) {
    ...
    if (this.players.map((_) => _.name).includes(player.name))
      throw new Error("Player already part of this Battle");
    if (this.players.push(player) === this.maxPlayerCount)
